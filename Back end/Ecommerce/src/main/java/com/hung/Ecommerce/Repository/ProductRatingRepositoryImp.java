...
@Repository
public class ProductRatingRepositoryImp implements ProductRatingRepository{
	...
	public ProductRating findById(int id, boolean fetchOrNot) {
		...
		CriteriaQuery<ProductRating> CQ = CB.createQuery(ProductRating.class);
		Root<ProductRating> root = CQ.from(ProductRating.class);
		
		CQ.select(root).where(CB.equal(root.get("id"), id));
		Query query = entityManager.createQuery(CQ);
		List<ProductRating> result =  query.getResultList();
		...
		if(fetchOrNot == true) {
			...
			for(String object: fetchObjects) {
				...
				CriteriaQuery<ProductRating> newCQ = newCB.createQuery(ProductRating.class);
				Root<ProductRating> newRoot = newCQ.from(ProductRating.class);
...
//			else{CQ.orderBy(CB.desc(root.get(propertyName)));}
	...
	@Override
	public List<ProductRating> findByProperties(Map<String, Object> conditions, Map<String, SelectionOrder> orders,
...
@Override
	public void reconnect(ProductRating player) {
			
		entityManager.refresh(player);
	}

@Override
public ProductRating update(ProductRating player) {
	
	return (ProductRating) entityManager.merge(player);
}
...
@Override
	public int updateByProperties(Map<String, Object> conditions, boolean TrueIsAnd_FalseIsOr, String targetField,
		...
		CriteriaBuilder CB = entityManager.getCriteriaBuilder(); 
		CriteriaUpdate<ProductRating> CD = CB.createCriteriaUpdate(ProductRating.class);
		Root<ProductRating> root = CD.from(ProductRating.class);
...
public void delete(int id) {	
	ProductRating reservation = entityManager.find(ProductRating.class, id);	
...
@Override
	public int deleteByProperties(Map<String, Object> conditions, boolean TrueIsAnd_FalseIsOr) {
		...
		CriteriaBuilder CB = entityManager.getCriteriaBuilder(); 
		CriteriaDelete<ProductRating> CD = CB.createCriteriaDelete(ProductRating.class);
		Root<ProductRating> root = CD.from(ProductRating.class);
