...
@Repository
public class AbstractRequestRepositoryImp implements AbstractRequestRepository{
...
public AbstractRequest findById(int id, boolean fetchOrNot) {
	...
	CriteriaQuery<AbstractRequest> CQ = CB.createQuery(AbstractRequest.class);
	Root<AbstractRequest> root = CQ.from(AbstractRequest.class);
	
	CQ.select(root).where(CB.equal(root.get("id"), id));
	
	Query query = entityManager.createQuery(CQ);
	List<AbstractRequest> result = query.getResultList();
...
@Override
	public List<AbstractRequest> findByProperties(Map<String, Object> conditions, Map<String, SelectionOrder> orders,
...
@Override
	public void reconnect(AbstractRequest abstractRequest) {
		...
		entityManager.refresh(abstractRequest);
	}

@Override
public AbstractRequest update(AbstractRequest abstractRequest) {
	
	return (AbstractRequest) entityManager.merge(abstractRequest);
}
...
@Override
	public int updateByProperties(Map<String, Object> conditions, boolean TrueIsAnd_FalseIsOr, String targetField,
		...
		CriteriaBuilder CB = entityManager.getCriteriaBuilder(); 
		CriteriaUpdate<AbstractRequest> CD = CB.createCriteriaUpdate(AbstractRequest.class);
		Root<AbstractRequest> root = CD.from(AbstractRequest.class);
...
public void delete(int id) {	
	AbstractRequest reservation = entityManager.find(AbstractRequest.class, id);	
...
@Override
	public int deleteByProperties(Map<String, Object> conditions, boolean TrueIsAnd_FalseIsOr) {
		...
		CriteriaBuilder CB = entityManager.getCriteriaBuilder(); 
		CriteriaDelete<AbstractRequest> CD = CB.createCriteriaDelete(AbstractRequest.class);
		Root<AbstractRequest> root = CD.from(AbstractRequest.class);
