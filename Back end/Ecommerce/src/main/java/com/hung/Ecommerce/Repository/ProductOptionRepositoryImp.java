...
@Repository
public class ProductOptionRepositoryImp implements ProductOptionRepository{
	...
	public ProductOption findById(int id, boolean fetchOrNot) {
		...
		List<ProductOption> result =  query.getResultList();
...
//			else{CQ.orderBy(CB.desc(root.get(propertyName)));}
	...
	@Override
	public List<ProductOption> findByProperties(Map<String, Object> conditions, Map<String, SelectionOrder> orders,
...
@Override
	public void reconnect(ProductOption player) {
			
		entityManager.refresh(player);
	}

@Override
public ProductOption update(ProductOption player) {
	
	return (ProductOption) entityManager.merge(player);
}
...
@Override
	public int updateByProperties(Map<String, Object> conditions, boolean TrueIsAnd_FalseIsOr, String targetField,
		...
		CriteriaBuilder CB = entityManager.getCriteriaBuilder(); 
		CriteriaUpdate<ProductOption> CD = CB.createCriteriaUpdate(ProductOption.class);
		Root<ProductOption> root = CD.from(ProductOption.class);
...
public void delete(int id) {	
	ProductOption reservation = entityManager.find(ProductOption.class, id);	
...
@Override
	public int deleteByProperties(Map<String, Object> conditions, boolean TrueIsAnd_FalseIsOr) {
		...
		CriteriaBuilder CB = entityManager.getCriteriaBuilder(); 
		CriteriaDelete<ProductOption> CD = CB.createCriteriaDelete(ProductOption.class);
		Root<ProductOption> root = CD.from(ProductOption.class);
