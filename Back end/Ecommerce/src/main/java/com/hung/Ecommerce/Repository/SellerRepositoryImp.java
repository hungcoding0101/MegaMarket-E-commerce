...
public Seller findById(int id, boolean fetchOrNot) {
	...
	if(fetchOrNot == true) {
		EntityGraph entityGraph = entityManager.getEntityGraph("seller.fetch.1");
...
@Override
public List<Seller> findByProperties(Map<String, Object> conditions, Map<String, SelectionOrder> orders,
...
@Override
	public void reconnect(Seller player) {
			
		entityManager.refresh(player);
	}

@Override
public Seller update(Seller player) {
	
	return (Seller) entityManager.merge(player);
}
...
@Override
	public int updateByProperties(Map<String, Object> conditions, boolean TrueIsAnd_FalseIsOr, String targetField,
		...
		CriteriaBuilder CB = entityManager.getCriteriaBuilder(); 
		CriteriaUpdate<Seller> CD = CB.createCriteriaUpdate(Seller.class);
		Root<Seller> root = CD.from(Seller.class);
...
public void delete(int id) {	
	Seller reservation = entityManager.find(Seller.class, id);	
...
@Override
	public int deleteByProperties(Map<String, Object> conditions, boolean TrueIsAnd_FalseIsOr) {
		...
		CriteriaBuilder CB = entityManager.getCriteriaBuilder(); 
		CriteriaDelete<Seller> CD = CB.createCriteriaDelete(Seller.class);
		Root<Seller> root = CD.from(Seller.class);
